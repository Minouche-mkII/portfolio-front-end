<script lang="ts">

    import type {GalleryImageDTO} from "../../../types/dto/page-dto";
    import {BACKEND_URL} from "$lib/config";
    import ImageCardComponent from "./ImageCardComponent.svelte";
    import {getContext} from "svelte";
    import type {OpenModalAction} from "../../../types/business/gallery/modal_action";

    type Props = {
        image: GalleryImageDTO,
    }
    let { image } : Props = $props()
    let style = $derived(`grid-column: span ${image.horizontal_span}; grid-row: span ${image.vertical_span};`)
    let imgRatio = $derived(`aspect-ratio:${image.horizontal_span} / ${image.vertical_span} ;`)
    const openModalAction = getContext("modal-card") as OpenModalAction

    let modal = $state(false)
    function openModal(event: MouseEvent) {
        const coordinates = (event.currentTarget as HTMLElement).getBoundingClientRect()
        const htmlImage = (event.currentTarget as HTMLElement).querySelector("img") as HTMLImageElement
        let origin = {
            x: coordinates.x,
            y: coordinates.y,
            mouseX: event.clientX,
            mouseY: event.clientY,
            baseWidth: coordinates.width,
            baseHeight: coordinates.height,
            naturalHeight: htmlImage.naturalHeight,
            naturalWidth: htmlImage.naturalWidth
        }
        modal = true
        openModalAction(origin, image, closeModal)
    }

    function closeModal() {
        modal = false
    }

</script>

<div role="button" class="image-preview" {style} tabindex="0" onclick={openModal}>
    <img style="{imgRatio}" src="{BACKEND_URL+image.src}" alt="{image.alt}" class="{modal ? 'hide': ''}">
</div>

<style>
    .image-preview {
        margin: 7px;
        cursor: pointer;
        user-select: none;
        transition: 0.2s ease;
    }
    .image-preview:hover {
        transform: scale(1.1);
        box-shadow: 0 0 1em 10px rgba(0, 0, 0, 0.9);
        z-index: 1000;
    }
    img {
        width: 100%;
        height: 100%;
    }
    .hide {
        opacity: 0;
    }
</style>
